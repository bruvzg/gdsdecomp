    ...
    with open(new_file_h, "w") as f:
        ...
        for bytecode_class in bytecode_classes:
            ...
            if bytecode_class.max_engine_version != "":
                line += (
                    bytecode_class.engine_version.strip("-stable")
                    + " - "
                    + bytecode_class.max_engine_version.strip("-stable")
                )
            else:
            ...
            line += (
                " ("
                + bytecode_class.bytecode_rev
                + " / "
                + bytecode_class.date
                + " / Bytecode version: "
                + str(bytecode_class.bytecode_version)
                + ") - "
                + generate_bytecode_description_string(bytecode_class)
                + '", '
                + str(bytecode_class.bytecode_version)
                + ", "
                + str(bytecode_class.is_dev).lower()
                + " },\n"
            )
            f.write(line)
        # ends with:
