...
def generate_class_header(dir: Path, bytecode_class:BytecodeClass) -> None:
    ...
    with open(new_file_h, "w") as f:
        ...
        if (len(new_added_tokens) > 0):
            f.write("\tvirtual Vector<GlobalToken> get_added_tokens() const override { return {" + ", ".join(new_added_tokens) + "}; }\n")
        if (len(new_removed_tokens) > 0):
            f.write("\tvirtual Vector<GlobalToken> get_removed_tokens() const override { return {" + ", ".join(new_removed_tokens) + "}; }\n")
        if (len(bytecode_class.added_functions) > 0):
            f.write("\tvirtual Vector<String> get_added_functions() const override { return {\"" + "\", \"".join(bytecode_class.added_functions) + "\"}; }\n")
        if (len(bytecode_class.removed_functions) > 0):
            f.write("\tvirtual Vector<String> get_removed_functions() const override { return {\"" + "\", \"".join(bytecode_class.removed_functions) + "\"}; }\n")
        if (len(bytecode_class.arg_count_changed) > 0):
            f.write("\tvirtual Vector<String> get_function_arg_count_changed() const override { return {\"" + "\", \"".join(bytecode_class.arg_count_changed) + "\"}; }\n")
        f.write("public:\n")
...
def generate_bytecode_description_string(bytecode_class:BytecodeClass) -> str:
    ...
    if len(bytecode_class.added_tokens) > 0:
        ...
        new_added_tokens = [token.replace("TK_PR_", "").replace("TK_CF_", "").replace("TK_", "") for token in bytecode_class.added_tokens]
            
        add_to_desc("added `{}` token", new_added_tokens)
    if len(bytecode_class.removed_tokens) > 0:
        new_removed_tokens = [token.replace("TK_PR_", "").replace("TK_CF_", "").replace("TK_", "") for token in bytecode_class.removed_tokens]
    ...
    if len(bytecode_class.tokens_renamed) > 0:
        for dict in bytecode_class.tokens_renamed:
            ...
            fmt_insert = "`{}` to `{}`".format(dict_key.replace("TK_PR_", "").replace("TK_CF_", "").replace("TK_", ""),
...
def generate_bytecode_version_header(dir: Path, bytecode_classes:list[BytecodeClass]) -> None:
    ...
    with open(new_file_h, "w") as f:
        ...
        f.write("#pragma once\n")
        f.write("\n")
        f.write("#include \"bytecode/bytecode_base.h\"\n")
        f.write("\n")
        for bytecode_class in bytecode_classes:
            f.write("#include \"bytecode/" + bytecode_class.file_stem + ".h\"\n")
        f.write("\n")
        f.write("void register_decomp_versions();\n")
        f.write("GDScriptDecomp *create_decomp_for_commit(uint64_t p_commit_hash);\n")
        f.write("Vector<Ref<GDScriptDecomp>> get_decomps_for_bytecode_ver(int bytecode_version, bool include_dev = false);\n")
        f.write("\n")
        f.write("\n")
        f.write("struct GDScriptDecompVersion {\n")
        f.write("\tuint64_t commit;\n")
        f.write("\tString name;\n")
        f.write("\tint bytecode_version;\n")
        f.write("\tbool is_dev;\n")
        f.write("};\n")
        f.write("\n")
        f.write("static const GDScriptDecompVersion decomp_versions[] = {\n")
        ...
        for bytecode_class in bytecode_classes:
            ...
            if not stable:
                line += "     "
            if (bytecode_class.max_engine_version != ""):
                line += bytecode_class.engine_version.strip("-stable") + " - " + bytecode_class.max_engine_version.strip("-stable")
            else:
                line += bytecode_class.engine_version.strip("-stable")
            if stable:
                line += " release"
            line += " (" + bytecode_class.bytecode_rev + " / " + bytecode_class.date + " / Bytecode version: " + str(bytecode_class.bytecode_version) + ") - " + generate_bytecode_description_string(bytecode_class) + "\", "+ str(bytecode_class.bytecode_version)+ ", "+ str(bytecode_class.is_dev).lower() + " },\n"
            f.write(line)
        #ends with:
        f.write("\t{ 0x0000000, \"-NULL-\", 0, false },\n")

        f.write("};\n")
        f.write("\n")
        f.write("\n")
        f.write("static const int num_decomp_versions = sizeof(decomp_versions) / sizeof(GDScriptDecompVersion);\n")
        f.write("\n")
        f.write("\n")
...
def generate_bytecode_versions_cpp(dir: Path, bytecode_classes:list[BytecodeClass]) -> None:
    ...
    with open(new_file_cpp, "w") as f:
        f.write(PRELUDE)
        f.write("#include \"bytecode_versions.h\"\n")
        f.write("\n")
        f.write("void register_decomp_versions() {\n")
        for bytecode_class in bytecode_classes:
            f.write("\tClassDB::register_class<" + bytecode_class.class_name + ">();\n")
        f.write("}\n")
        f.write("\n")
        f.write("GDScriptDecomp *create_decomp_for_commit(uint64_t p_commit_hash) {\n")
        # big giant case statement
        f.write("\tswitch(p_commit_hash) {\n")
        for bytecode_class in bytecode_classes:
            f.write("\t\tcase 0x" + bytecode_class.bytecode_rev + ": return memnew(" + bytecode_class.class_name + ");\n")
        f.write("\t\tdefault: return nullptr;\n")
        f.write("\t}\n")
        f.write("}\n")
        f.write("\n")
        f.write("Vector<Ref<GDScriptDecomp>> get_decomps_for_bytecode_ver(int bytecode_version, bool include_dev) {\n")
        f.write("\tVector<Ref<GDScriptDecomp>> decomps;\n")
        f.write("\tfor (int i = 0; i < num_decomp_versions; i++) {\n")
        f.write("\t\tif (decomp_versions[i].bytecode_version == bytecode_version && (include_dev || !decomp_versions[i].is_dev)) {\n")
        f.write("\t\t\tdecomps.push_back(Ref<GDScriptDecomp>(create_decomp_for_commit(decomp_versions[i].commit)));\n")
        f.write("\t\t}\n")
        f.write("\t}\n")
        f.write("\treturn decomps;\n")
        f.write("}\n")
